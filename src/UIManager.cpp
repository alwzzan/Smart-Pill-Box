...
void UIManager::displayHome(Time12H time, int16_t minutesToNextDose,
                            ...
                            bool wifiOn, bool muteOn, uint16_t lidOpenings) {
    if (!displayOn) return;
    ...
    if (minutesToNextDose >= 0) {
        ...
        drawCenteredText(buffer, 38);
    } else {
        drawCenteredText("No doses scheduled", 38);
    }
    
    // Progress
    char progressStr[16];
    sprintf(progressStr, "Today: %d/%d", dosesTaken, totalDoses);
    drawCenteredText(progressStr, 47);
    
    // Lid opened count (independent from doses)
    char lidStr[20];
    sprintf(lidStr, "Lid opened: %d", lidOpenings);
    drawCenteredText(lidStr, 56);
...
void UIManager::displayDoseEdit(Time12H time, uint8_t editField, bool isNew, const char* titleOverride) {
    if (!displayOn) return;
    ...
    display.setTextSize(1);
    if (titleOverride) {
        drawCenteredText(titleOverride, 0);
    } else {
...
void UIManager::displayTimeEdit(Time12H time, uint8_t editField) {
    displayDoseEdit(time, editField, false, "SET TIME");
}
