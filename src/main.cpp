...
uint16_t editYear;
bool doseEditListMode = true;  // For handleDoseEdit list/editor toggle
...
void initializeSystem() {
    ...
    if (!uiManager.begin()) {
        ...
        delay(3000);
        ESP.restart();  // Restart instead of hanging forever
    }
    ...
    if (!timeManager.begin()) {
        ...
        for (int i = 0; i < 30; i++) { delay(100); yield(); }
    }
...
void loop() {
    ...
    uiManager.update();
    webServer.update();  // Drive WiFi state machine
...
void handleHomeScreen() {
    ...
    if (millis() - lastDisplayUpdate >= 1000) {
        ...
        uiManager.displayHome(currentTime, minutesToNext, takenCount, totalCount,
                             webServer.isRunning(), systemState.muteMode,
                             lidSensor.getOpeningsToday());
    }
    ...
    if (okEvent == BTN_SHORT_PRESS) {
        ...
        if (!webServer.isBusy()) {
            if (webServer.isOff()) {
                systemState.wifiEnabled = true;
                webServer.requestStart();
                alarmController.playConfirm();
            } else if (webServer.isRunning()) {
                systemState.wifiEnabled = false;
                webServer.requestStop();
            }
        }
    }
        ...
        switch (systemState.menuSelection) {
            case 0:  // Add Dose
                if (doseManager.getDoseCount() >= MAX_DOSES) {
                    ...
                    for (int i = 0; i < 15; i++) { delay(100); yield(); }
                } else {
            ...
            case 1:  // Edit Dose
                if (doseManager.getDoseCount() == 0) {
                    ...
                    doseEditListMode = true;  // Always start in list mode
                }
                break;
            case 2:  // Delete Dose
                if (doseManager.getDoseCount() == 0) {
...
void handleWiFiToggle() {
    // Show actual running state, not just the requested flag
    bool actuallyRunning = webServer.isRunning();
    String ip = actuallyRunning ? webServer.getIPAddress() : "";
    uiManager.displayWiFiToggle(actuallyRunning, ip.c_str());
    
    ButtonEvent okEvent = buttonHandler.getOkEvent();
    if (okEvent == BTN_SHORT_PRESS && !webServer.isBusy()) {
        if (webServer.isOff()) {
            systemState.wifiEnabled = true;
            webServer.requestStart();
        } else if (webServer.isRunning()) {
...
void goToHome() {
    systemState.currentMenu = MENU_HOME;
    systemState.menuSelection = 0;
    doseEditListMode = true;  // Reset for next entry
    uiManager.updateActivity();
}
